...
# ==================== CONFIGURATION ====================
BOT_NAME = "Shards of Orr Testing"
...
def farm_bds_routine(bot_instance: Botting) -> None:
    ...
    # Disable combat before path_after_door — enemies are in this area
...
def _enable_torch_runner_custom_behaviors() -> Generator:
    """Re-enable CustomBehaviors on the torch runner after passing through the portal."""
    my_email = Player.GetAccountEmail()
    GLOBAL_CACHE.ShMem.SendMessage(my_email, TORCH_RUNNER_EMAIL, SharedCommandType.EnableWidget, (0, 0, 0, 0), ("CustomBehaviors", "", "", ""))
    ConsoleLog(BOT_NAME, "[Portal Push] Re-enabled CustomBehaviors on the torch runner")
    yield
...
def _disable_party_member_behind() -> Generator:
    ...
    ConsoleLog(BOT_NAME, "[Torch Split] Disabled OnPartyMemberBehind and OnPartyMemberDeadBehind callbacks")
    yield
...
def _party_waypoint_manager() -> Generator:
    ...
    for x, y in path2_part2:
        ...
        # Phase 1: wait for slot 2 to enter combat (timeout 5s if no enemies on this leg)
        enter_deadline = time.monotonic() + 5.0
        while time.monotonic() < enter_deadline:
            ...
            for acc in accounts:
                if acc.AgentPartyData.PartyPosition == 1:
                    slot2 = acc
                    break
            if slot2 is not None and slot2.AgentData.Is_InCombatStance:
                ConsoleLog(BOT_NAME, f"[Torch Split] Slot 2 entered combat near ({x}, {y})")
                break
            yield

        # Phase 2: wait until slot 2 is out of combat — poll every 500ms instead of every yield
        last_poll = time.monotonic()
        cached_accounts = GLOBAL_CACHE.ShMem.GetAllAccountData()
        while True:
            now = time.monotonic()
            ...
            for acc in cached_accounts:
                if acc.AgentPartyData.PartyPosition == 1:
                    slot2 = acc
                    break
            if slot2 is None or not slot2.AgentData.Is_InCombatStance:
                dead_status = slot2.AgentData.Is_Dead if slot2 is not None else "N/A"
                ConsoleLog(BOT_NAME, f"[Torch Split] Slot 2 cleared combat, advancing (slot2_dead={dead_status})")
                break
            yield
...
def _brazier_sequence_gen(points: list[tuple[float, float]], interact_dist: float = 200.0) -> Generator:
    ...
    while idx < total:
        ...
        def _go_relight():
            ...
            ConsoleLog(BOT_NAME, f"[BRAZIER] Returning to last lit brazier {last_lit + 1}/{total} to re-light torch (the torch runner pos: {px:.0f},{py:.0f}, holding={holding_before})")
